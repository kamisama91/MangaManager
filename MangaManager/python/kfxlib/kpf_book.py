...
class KpfBook(object):
    def fix_kpf_prepub_book(self, fix_book, retain_yj_locals):
        ...
        if added_cover_eid is not None:
            for book_navigation in self.fragments["$389"].value:
                add_cover_landmark = True
                landmark_nav_container = None
                ...
                for nav_container in nav_containers:
                    ...
                    if nav_container.get("$235", None) == "$236":
                        landmark_nav_container = nav_container
                        for entry in nav_container.get("$247", []):
                            if unannotated(entry).get("$238") == "$233":
                                add_cover_landmark = False
                                break
                        break
