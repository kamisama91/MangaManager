...
class BookStructure(object):
    ...
    def check_consistency(self):
        ...
        if self.is_scribe_notebook:
            required_ftypes.remove("$389")
            required_ftypes.remove("$611")
            allowed_ftypes.add("$611")
        ...
        if self.has_pdf_resource:
            ...
            if self.get_feature_value("yj_pdf_backed_fixed_layout") is None:
        ...
        if not self.is_scribe_notebook:
            self.check_position_and_location_maps()

        if self.kpf_container is not None and self.kpf_container.kcb_data is not None:
    ...
    def check_fragment_usage(self, rebuild=False, ignore_extra=False):
        ...
        if self.is_kpf_prepub:
            for fragment in list(unreferenced_fragments):
                if fragment.ftype in ["$391", "$266", "$259", "$260", "$608"]:
    ...
    def has_illustrated_layout_page_template_condition(self):
        def _scan_section(data):
            data_type = ion_type(data)

            if data_type is IonAnnotation:
                return _scan_section(data.value)
