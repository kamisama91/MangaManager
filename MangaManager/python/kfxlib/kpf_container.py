...
class KpfContainer(YJContainer):
    ...
    def process_db_fragment(self, id, payload_type, payload_value):
        ftype = id
        element_type = self.element_type.get(id)
        ...
        if payload_type == "blob":
            ...
            if payload_data is None:
                ...
                if not self.have_symbol_table_or_max_id:
                    ...
                    self.have_symbol_table_or_max_id = True
                ...
                if element_type == "auxiliary_data":
                    ftype = "$597"
                    fid = self.create_local_symbol(id)
                    self.fragments.append(YJFragment(ftype=ftype, fid=fid, value=IonStruct({
                        IS("$598"): fid, IS("$258"): IonList()})))
                elif id == "book_navigation":
                    pass
                else:
