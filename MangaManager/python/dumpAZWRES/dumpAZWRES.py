...
def main(input_file, output_folder):
    ...
    infileext = os.path.splitext(input_file)[1].upper()
    if infileext not in ['.AZW6', '.RES']:
        print("Error: first parameter must be a Kindle AZW6 HD container file with extension .azw6 or .res.")
        return 1
        ...
        for i in range(num_sections):
            ...
            if dtext in dtmap2.keys():
                ...
                if dtag == "CONT":
                    ...
                    process_CRES(i, data, output_folder)
            else:
                desc = dtext
            if desc != "CONT":
